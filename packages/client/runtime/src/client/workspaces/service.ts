...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  async createDirectory(root: string | undefined, path: string, name: string): Promise<string> {
    const response = await this.api.host.createDirectory(root === undefined ? { path, name } : { root, path, name })
    if (!response.result.ok) throw new DirectoryBrowseError(response.result.error)
    return response.result.value.path
  }
  ...
  async readTextFile(root: string | undefined, path: string, maxBytes?: number, signal?: AbortSignal): Promise<{ path: string; text: string; truncated: boolean }> {
    const response = await this.api.host.readTextFile(
      root === undefined
        ? maxBytes === undefined ? { path } : { path, maxBytes }
        : maxBytes === undefined ? { root, path } : { root, path, maxBytes },
      signal,
    )
    if (!response.result.ok) throw new DirectoryBrowseError(response.result.error)
    return response.result.value
